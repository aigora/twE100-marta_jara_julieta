...
void nombreValido(char nombre[50]) {
	...
	do {
		...
		if (longitud < 3 || longitud > 10) { //Si no tiene entra 3 y 10 caracteres
			final = -2;
		}
		else {
			for (i = 0; nombre[i] != '\0'; i++) {

				if ((nombre[i] < 'a' || nombre[i]> 'z') && (nombre[i] < 'A' || nombre[i]> 'Z')) {//No hay letras
					final = -1;
				}
				else {
					final = 1;
		...
		if (final == -2) {
			printf("El nombre debe de ser de entre 3 y 10 caracteres, introduzca de nuevo un nombre\n");
		}
		else if (final == -1) {
...
void telefonoValido(char telefono[10]) {
	int i, contador, longitud, final;

	do {
		scanf_s("%s", telefono, 50);
